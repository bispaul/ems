...
def conv_for_db(datadict, discom, state):
    ...
    for rownum in xrange(1, 97):
        row = []
        date = dateutil.parser.parse(datadict.get('DATE'), dayfirst=True)
        row.append(date)
...
def update_db(dsn, data):
    ...
    sql = """INSERT INTO `power`.`internal_drawl_schedule_stg`
             (declared_date,
              date,
              revision,
              block_no,
...
def get_generator_schedule(dsn, date, discom='GUVNL',
                           state='GUJARAT', pool_name='INTERNAL'):
    ...
    url = "https://www.sldcguj.com/Operation/DECLARED_CAPABILITYNew1.php"
    url2 = \
        "https://www.sldcguj.com/Operation/DECLARED_CAPABILITY_report_new1.php"
    ...
    table4 = petl.addfield(table3, 'GENERATION_TYPE', None)
    table5 = petl.addfield(table4, 'POOL_NAME', pool_name)
    dbloaddata = list(table5)[1:]
...
# get_generator_schedule(dsn, '05-06-2017')
