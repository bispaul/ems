...
def data_prep_forecast_nn(config, discom, state):
    ...
    powercut_table = \
        pd.read_sql_query("""select date, block_no,
    ...
    powercut_table.sort_values(by=['date', 'block_no'], ascending=[True, True],
                               inplace=True)
    load_table_initial = \
        pd.read_sql_query("""select date, block_no, constrained_load
    ...
    # current_day_load = load_table_initial[load_table_initial['date']== max_date]
    # current_day_load = current_day_load[np.isfinite(current_day_load['reported_load'])]
    ...
    def getEnvelopeModels(aTimeSeries, delta, rejectCloserThan=0):
        ...
        for k in xrange(1, len(aTimeSeries) - delta):
            # Mark peaks
            if (sign(aTimeSeries[k] - aTimeSeries[k - delta]) in (0, 1)) and\
               (sign(aTimeSeries[k] - aTimeSeries[k + delta]) in (0, 1)) and\
...
for i in xrange(0, len(uniquen_location)):
    ...
    weather_data_temp_block = weather_data_temp[
        weather_data_temp['hour'] <= max_hour]
    weather_summary = \
    ...
    weekday_correction['week_day_correction_factor'] = \
        np.where((weekday_correction['dayofweek'] == 6) |
                 (weekday_correction['dayofweek'] == 5) |
                 (weekday_correction['dayofweek'] == 0),
                 weekday_correction['week_day_correction_factor'],
                 weekday_correction['week_day_correction_factor'])
    weather_dist_lag_initial['NN_dayofweek'] = pd.DatetimeIndex(
        weather_dist_lag_initial['lag_date']).dayofweek
    ...
    weather_dist_lag_initial_NNDOW['endo_demand_NN_DOW'] = \
        ...
        (1 - weather_dist_lag_initial_NNDOW['week_day_correction_factor'])
    ...
    weather_dist_lag_initial_NNDOWEVENT = \
        pd.merge(weather_dist_lag_initial_NNDOW_EVENT,
                 event_correction_summary,
                 how='left',
                 on=['name', 'block_no'])

    weather_dist_lag_initial_NNDOWEVENT['event_day_correction_factor_median'].\
        fillna(0, inplace=True)
    weather_dist_lag_initial_NNDOWEVENT['endo_demand_NN_event'] = \
        weather_dist_lag_initial_NNDOWEVENT['endo_demand'] * \
        (1 - weather_dist_lag_initial_NNDOWEVENT[
         'event_day_correction_factor_median'])
    ...
    weekday_correction.sort_values(by=['month', 'dayofweek', 'block_no'],
                                   ascending=[True, True, True],
                                   inplace=True)
    data_forecast_weekday = pd.merge(weather_dist_lag, weekday_correction,
                                     how='left',
                                     on=['month', 'dayofweek', 'block_no'])
    data_forecast_weekday['weekend_correction_smooth'] = \
        data_forecast_weekday['week_day_correction_factor'].\
    ...
    for j in xrange(0, len(unique_event)):
        test_event = event_day_residual_summary[
            event_day_residual_summary['name'] == unique_event[j]]
        test_event.sort_values(by=['block_no'],
                               ascending=[True],
                               inplace=True)
        test_event['event_correction_smooth'] = \
            test_event['event_residual_factor_median'].\
            rolling(window=7, center=True).mean()
        test_event['event_correction_smooth'].\
            fillna(method="ffill", inplace=True)
        test_event['event_correction_smooth'].\
            fillna(method="bfill", inplace=True)
        test_event['name'] = unique_event[j]
        event_correction_smooth = event_correction_smooth.append(test_event)

    event_correction_smooth = event_correction_smooth[
        ['name', 'block_no', 'event_correction_smooth']]
...
# data_prep_forecast_uw(config, 'GUVNL')
