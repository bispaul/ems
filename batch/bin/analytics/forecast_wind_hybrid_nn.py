...
def forecast_wind_hybrid_nn(config, discom, state):
    re_type = "WIND"
    ...
    WIND_generation_table = pd.read_sql_query("""select
        generator_name, date, block_no, generation
        from generation_staging where pool_type = '{}'
        and discom = '{}'""".format(re_type, discom), engine, index_col=None)
    ...
    WS_table = WS_table[['location', 'date', 'hour', 'windspeed_actual',
                         'windspeed_forecast',
    ...
    window_size = 6
    WIND_forecast_nn_WS_final['window'] = np.ceil(WIND_forecast_nn_WS_final['block_no']/window_size)
    window_fore_max = (WIND_forecast_nn_WS_final.groupby(['date','window'],as_index=False)
                                    .agg({'simday_WIND_WS_MW_hourly':{'max','min'}}))
    window_fore_max.columns = ['_'.join(col).strip() for col in window_fore_max.columns.values]
    window_fore_max.rename(columns={'date_':'date',
                                    'window_':'window',
                                    'simday_WIND_WS_MW_hourly_max': 'forecast_max',
                                    'simday_WIND_WS_MW_hourly_min': 'forecast_min'}, inplace=True)
    window_fore_block = pd.merge(window_fore_max,
                                 WIND_forecast_nn_WS_final, 
                                 how = 'left',
                                 on = ['date','window'])

    wind_forecast_smooth = pd.DataFrame([])
    unique_date = window_fore_block['date'].unique()
    for j in range(0, len(unique_date)):
        test = window_fore_block.loc[window_fore_block['date'] ==
                                     unique_date[j]]
        s = np.array(test['forecast_max'])
        yhat = savitzky_golay(s, window_size=35, order=2)
        envelop = pd.DataFrame(yhat)
        envelop = envelop.iloc[0:]
        test['smooth_forecast'] = envelop.values
        wind_forecast_smooth = wind_forecast_smooth.append(test)

    smooth_WIND_forecast = \
        wind_forecast_smooth[['date', 'block_no', 'gen_MW',
                              'prev_day_capacity_sum_mean',
                              'per_gen',
                              'smooth_forecast']]

    act_wind_gen = WIND_generation_table.\
        groupby(['date', 'block_no'], as_index=False).\
        agg({'generation': 'sum'})

    act_wind_gen['quantum'] = act_wind_gen['generation'] * 4
    act_wind_gen = \
        act_wind_gen[np.isfinite(act_wind_gen['quantum'])]
    wind_gen_smooth = pd.DataFrame([])
    unique_date = act_wind_gen['date'].unique()
    for j in range(0, len(unique_date)):
        test = act_wind_gen[act_wind_gen['date'] ==
                            unique_date[j]]
    ...
    pred_table = pd.merge(smooth_WIND_forecast, wind_gen_smooth,
                          how='left', on=['date', 'block_no'])
    pred_table['new_smooth_forecast'] = \
    ...
    for j in range(0, 100):
        pred_table['new_smooth_forecast'] = \
            pred_table['new_smooth_forecast'].rolling(window=5,
                                                      center=True,
                                                      axis=0).mean()
        test = pred_table.copy()
        test['mape'] = abs(test['quantum_smooth'] /
                           test['new_smooth_forecast'] - 1)
        test['mape1'] = abs(test['quantum_smooth'] /
    ...
    # smooth_WIND_forecast['smooth_forecast'] = \
    #     np.where((smooth_WIND_forecast['smooth_forecast'] >= 3800), 3800,
    #              smooth_WIND_forecast['smooth_forecast'])
    # smooth_WIND_forecast['smooth_forecast'] = \
    #     np.where((smooth_WIND_forecast['smooth_forecast'] <= 200), 200,
    #              smooth_WIND_forecast['smooth_forecast'])

    # Pred_table_wind_NN = \
    #     smooth_WIND_forecast[['date', 'block_no', 'smooth_forecast']]
    Pred_table_wind_NN = \
        pred_table[['date', 'block_no', 'new_smooth_forecast']].copy()
    Pred_table_wind_NN.rename(columns={'new_smooth_forecast': 'gen_forecast'},
                              inplace=True)
    Pred_table_wind_NN['org_name'] = discom
    Pred_table_wind_NN['pool_name'] = 'INT_GENERATION_FOR'
    Pred_table_wind_NN['pool_type'] = re_type
    Pred_table_wind_NN['entity_name'] = discom
    Pred_table_wind_NN['state'] = state
    Pred_table_wind_NN['revision'] = 0
    Pred_table_wind_NN['model_name'] = 'HYBRID_NN'

    tablename = 'Pred_table_wind_HYBRID_NN_{}'.format(discom)
    Pred_table_wind_NN.to_sql(con=engine, name=tablename, if_exists='replace',
...
discom = "GUVNL"
state = "GUJARAT"
forecast_wind_hybrid_nn(config, discom, state)