"""HybridDLN Model  GUVNL."""
from sqlalchemy import create_engine
import pandas as pd
# import numpy as np


def forecast_hybriddln_guvnl(config, discom, state):
    """Forecast HYBRIDDLN GUVNL."""
    ...
    pred_table['HYBRID_PRED_DEMAND'] = pred_table[['MLP_PRED_DEMAND',
                                                   'SVR_PRED_DEMAND',
                                                   'DLN_PRED']].median(axis=1)
    pred_table['mape'] = abs(pred_table['endo_demand'] -
                             pred_table['HYBRID_PRED_DEMAND']) / \
        pred_table['endo_demand']

    # pred_table['mape'].describe()
    ...
    pred_table_HYBRID['model_name'] = 'HYBRID_DLN'
    tablename = 'pred_table_HYBRIDDLN_{}'.format(discom)
